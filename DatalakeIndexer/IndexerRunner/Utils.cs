...
public static class Utils
{
    ...
    /// </summary>
    /// <returns></returns>
    public static IEnumerable<PathRowType> GetMockPaths(
        int fileSystemCount = 10,
        int directoryCount = 100,
        int fileCount = 100)
    {
        ...
        var paths = Enumerable.Range(0, fileCount).Select(o => $"somepath_{o}");

        var now = DateTime.UtcNow;

        return filesystems
            .SelectMany(f => directories
                .SelectMany((d, i) => paths.Select(p => $"{d}/{p}.{GetRandomFilePath()}")
